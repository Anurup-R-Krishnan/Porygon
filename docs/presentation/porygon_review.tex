...
\begin{frame}
    ...
    {\large \textbf{\textcolor{amritaMaroon}{POR\kern0.5pt YGON --- Learning What ``Normal'' Looks Like\\ Inside a Running Container}}} \\
    \vspace{0.08cm}
    {\scriptsize\itshape Does knowing \emph{how} a container was deployed make its behaviour easier to judge?} \\
    \vspace{0.12cm}
...
\medskip
\textbf{The catch.} To compare today against ``normal'', you must first decide
\emph{which normal}. Group the wrong containers together and the picture blurs.
...
\begin{tikzpicture}[node distance=4mm]
  ...
  \node[box, below=6mm of odd, minimum width=34mm, blue] (q) {Is this an attack,\\ or a Tuesday maintenance job?};
  \draw[fl] (app) -- (norm);
  \draw[fl] (norm) -- (odd);
  \draw[fl] (odd) -- (q);
\end{tikzpicture}
...
\smallskip
{\scriptsize The honest answer is often ``we cannot tell yet''.\\ Our design keeps that
...
\small
Every container image has a \textbf{content fingerprint} --- a long code computed
from the exact bytes of the image. Same code, same program. It cannot be faked
or quietly changed.

\medskip
So: group containers by fingerprint, learn one ``normal'' per fingerprint. Done?

\medskip
\textbf{Not quite.} The same image can be \emph{deployed} in different ways ---
different start-up command, different user account, different permissions,
different folders attached. Those differences may change behaviour.

\medskip
\textcolor{amritaMaroon}{\textbf{Our question:}} does adding \emph{how it was
deployed} to the grouping give a cleaner ``normal'' --- or just split the data
into pieces too small to learn from?
...
\smallskip
{\scriptsize We compare \textbf{four} ways of grouping and let the measurements
decide. A ``no difference'' answer is a valid result, not a failure.}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[node distance=3.5mm]
  ...
  \node[box, right=of k, minimum width=21mm] (n) {Clean up\\\& de-duplicate};
  ...
  \node[lbl, below=1.5mm of k, text width=21mm, align=center] {kernel-level,\\no code changes};
  ...
  \node[lbl, below=1.5mm of h, text width=20mm, align=center] {nothing acts\\on its own};
\end{tikzpicture}}
...
\footnotesize
\textbf{Two kinds of evidence, kept apart on purpose}
...
\end{itemize}
Mixing them into one number hides which one fired.

\column{0.48\textwidth}
\footnotesize
\textbf{What is deliberately \emph{not} in the loop}
\begin{itemize}\setlength\itemsep{1pt}
  \item No automatic shutting down of containers
  \item No machine-learning black box
  \item No claim that ``unusual'' means ``attack''
\end{itemize}
\end{columns}
\end{frame}
...
Unseen pairs get a small non-zero chance, so nothing divides by zero.

\medskip
\textbf{Step 3 --- How much is brand new?}\\
The share of activity whose program was \emph{never} seen while learning.

\column{0.45\textwidth}
\footnotesize
\textbf{Step 4 --- Combine, without invented weights}\\
Each measure is turned into a rank against known-normal runs, then \textbf{averaged
equally}. If a measure is missing, it is recorded as missing --- not guessed.

\medskip
\textbf{Step 5 --- Turn it into a fair scale}\\
Take $n$ complete known-normal runs held back and never used for learning:
\vspace{-1mm}
\[ p=\frac{1+\#\{\text{normal runs at least this odd}\}}{n+1} \]
\vspace{-3mm}

\smallskip
\textcolor{amritaMaroon}{\textbf{Read it as:}} ``about 1 in 20 normal runs look
this unusual'' --- \emph{not} ``95\% chance of attack''.

\medskip
\begin{tikzpicture}
  \node[box, amber, text width=48mm, font=\tiny]
  {\textbf{Why no tuned weights?} An earlier version mixed the measures using
   $0.50$, $0.30$, $0.20$. Nobody could justify $0.50$ over $0.45$. Tuning those
   numbers to make results look good is exactly the mistake we are avoiding, so
   the code now refuses to accept weights at all.};
\end{tikzpicture}
\end{columns}
\end{frame}
...
Unusual-but-harmless jobs (backups, restarts, maintenance) are recorded too ---
those are the cases that cause false alarms.

\smallskip
\textbf{Splitting rule:} a \emph{whole run} goes to learning, calibration, or
testing --- never split inside a run. Snapshots seconds apart are near-copies;
treating them as independent would make our confidence look better than it is.
...
\smallskip
\textcolor{amritaMaroon}{\textbf{Rule we hold ourselves to:}} every percentage is
reported with the actual counts behind it. No number appears without the runs
that produced it.
\end{columns}
\end{frame}
...
\midrule
Kernel-level watcher      & Every program start, in every container   & \ok{Working} \\
...
Vulnerability lookup      & Ingredient list per image; known-exploited flags     & \ok{Working} \\
...
evaluation $\approx$15\%. We report the split, not one flattering number.}
\end{frame}

% ============================================================
% RUBRIC 2b: EVIDENCE
% ============================================================
\begin{frame}{2. Implementation Progress --- Evidence It Actually Runs}
...
Seen by the kernel watcher & \ok{96} \\
...
\begin{tikzpicture}[node distance=2.2mm]
  ...
  \node[box, right=of a, minimum width=12mm] (b) {kernel};
...
\framesubtitle{Live demonstration: the refusals are the interesting part}

\begin{columns}[T]
\column{0.5\textwidth}
\footnotesize
A system like this is only trustworthy if it says \textbf{no} at the right
moments. Three refusals we can show live:
...
\smallskip
{\scriptsize Each refusal is enforced by code and covered by a test, so it cannot
...
& Same evidence, far less risk than writing our own kernel code
& Building a custom kernel sensor --- large effort, large attack surface \\
...
\framesubtitle{Every choice costs something; here is the bill}
...
\footnotesize
\textbf{1. The pipeline loses nothing (so far).}\\
96 marked events created, 96 seen by the kernel watcher, 96 stored. Points we
\emph{cannot} yet measure are labelled ``not measured'' --- never assumed to be zero.

\medskip
\textbf{2. A bug that only two runs could reveal.}\\
...
\medskip
\textbf{3. Serving traffic starts no programs.}\\
40 web requests and 40 database-cache operations produced \emph{zero} recorded
program starts. What we profile is start-up and maintenance activity --- worth
...
\smallskip
{\scriptsize Catching this early is exactly what a trial run is for.}
\end{columns}
\end{frame}
...
\vspace{5mm}
\footnotesize
\textbf{The contribution we are aiming for} is not ``we watched containers'' ---
that is well-established work. It is a careful, repeatable comparison of
\emph{four different ways of deciding what counts as the same thing}, reported
honestly whichever way it comes out.

\smallskip
Every number will be traceable to the exact run that produced it, and every run
is stored so a reviewer can recompute the tables themselves.
\end{frame}
...
\begin{frame}
    ...
    \vspace{6mm}
    {\footnotesize Questions welcome --- including the uncomfortable ones about what\\
    this system cannot yet prove.}
\end{frame}
