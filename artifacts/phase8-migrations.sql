...
-- Running upgrade 0006_phase6 -> 0007_phase7
...
CREATE TABLE response_recommendations (
    ...
    decision_note TEXT, 
    approved_action VARCHAR(32), 
    PRIMARY KEY (recommendation_id), 
    CONSTRAINT ck_response_recommendations_status CHECK (status IN ('proposed', 'approved', 'rejected')), 
    CONSTRAINT ck_response_recommendations_action CHECK (recommended_action IN ('observe_only', 'pause_container', 'stop_container')), 
    CONSTRAINT ck_response_recommendations_approved_action CHECK (approved_action IS NULL OR approved_action IN ('observe_only', 'pause_container', 'stop_container')), 
    FOREIGN KEY(incident_id) REFERENCES incidents (incident_id) ON DELETE RESTRICT, 
    CONSTRAINT uq_response_recommendations_key UNIQUE (recommendation_key)
);

CREATE INDEX ix_response_recommendations_incident ON response_recommendations (incident_id, created_at);

CREATE INDEX ix_response_recommendations_status ON response_recommendations (status);

CREATE INDEX ix_response_recommendations_target ON response_recommendations (target_container_id);
...
CREATE TABLE response_executions (
    ...
    pre_state JSON NOT NULL, 
    post_state JSON NOT NULL, 
    result JSON NOT NULL, 
    ...
    rollback_requested_by VARCHAR(128), 
    rollback_note TEXT, 
    rollback_started_at TIMESTAMP WITH TIME ZONE, 
    rollback_completed_at TIMESTAMP WITH TIME ZONE, 
    PRIMARY KEY (execution_id), 
    CONSTRAINT ck_response_executions_status CHECK (status IN ('pending', 'claimed', 'succeeded', 'failed', 'rollback_pending', 'rollback_claimed', 'rolled_back', 'rollback_failed')), 
    CONSTRAINT ck_response_executions_action CHECK (action_type IN ('observe_only', 'pause_container', 'stop_container')), 
    FOREIGN KEY(incident_id) REFERENCES incidents (incident_id) ON DELETE RESTRICT, 
    FOREIGN KEY(recommendation_id) REFERENCES response_recommendations (recommendation_id) ON DELETE RESTRICT, 
    CONSTRAINT uq_response_executions_recommendation UNIQUE (recommendation_id), 
    CONSTRAINT uq_response_executions_idempotency UNIQUE (idempotency_key)
);

CREATE INDEX ix_response_executions_status_created ON response_executions (status, created_at);

CREATE INDEX ix_response_executions_incident ON response_executions (incident_id);

CREATE INDEX ix_response_executions_target ON response_executions (target_container_id);

CREATE INDEX ix_response_executions_lease ON response_executions (lease_expires_at);
...
CREATE TABLE response_audit_events (
    ...
    FOREIGN KEY(execution_id) REFERENCES response_executions (execution_id) ON DELETE SET NULL, 
    FOREIGN KEY(incident_id) REFERENCES incidents (incident_id) ON DELETE RESTRICT, 
    FOREIGN KEY(recommendation_id) REFERENCES response_recommendations (recommendation_id) ON DELETE SET NULL
);

CREATE INDEX ix_response_audit_incident_time ON response_audit_events (incident_id, created_at);

CREATE INDEX ix_response_audit_recommendation ON response_audit_events (recommendation_id);

CREATE INDEX ix_response_audit_execution ON response_audit_events (execution_id);

CREATE INDEX ix_response_audit_type ON response_audit_events (event_type);

UPDATE alembic_version SET version_num='0007_phase7' WHERE alembic_version.version_num = '0006_phase6';

-- Running upgrade 0007_phase7 -> 0008_phase8
...
CREATE TABLE image_scans (
    ...
    scan_reference VARCHAR(128) NOT NULL, 
    status VARCHAR(32) NOT NULL, 
    ...
    error_code VARCHAR(64), 
    error_message TEXT, 
    scanner_metadata JSON NOT NULL, 
    summary JSON NOT NULL, 
    PRIMARY KEY (scan_id), 
    CONSTRAINT ck_image_scans_status CHECK (status IN ('queued', 'claimed', 'completed', 'failed')), 
    CONSTRAINT uq_image_scans_key UNIQUE (scan_key)
);

CREATE INDEX ix_image_scans_digest_created ON image_scans (image_digest, created_at);

CREATE INDEX ix_image_scans_status_created ON image_scans (status, created_at);

CREATE INDEX ix_image_scans_lease ON image_scans (lease_expires_at);
...
CREATE TABLE sbom_artifacts (
    ...
    CONSTRAINT uq_sbom_artifacts_scan UNIQUE (scan_id)
);

CREATE INDEX ix_sbom_artifacts_image_digest ON sbom_artifacts (image_digest);

CREATE TABLE vulnerability_report_artifacts (
    artifact_id VARCHAR(36) NOT NULL, 
    scan_id VARCHAR(36) NOT NULL, 
    image_digest VARCHAR(255) NOT NULL, 
    format VARCHAR(32) NOT NULL, 
    ...
    PRIMARY KEY (artifact_id), 
    FOREIGN KEY(scan_id) REFERENCES image_scans (scan_id) ON DELETE CASCADE, 
    CONSTRAINT uq_vulnerability_report_artifacts_scan UNIQUE (scan_id)
);

CREATE INDEX ix_vulnerability_report_artifacts_digest ON vulnerability_report_artifacts (image_digest);

CREATE TABLE vulnerability_intel (
    cve_id VARCHAR(32) NOT NULL, 
    epss_score FLOAT, 
    epss_percentile FLOAT, 
    epss_date VARCHAR(10), 
    kev BOOLEAN NOT NULL, 
    kev_date_added VARCHAR(10), 
    kev_due_date VARCHAR(10), 
    kev_vendor_project VARCHAR(255), 
    kev_product VARCHAR(255), 
    kev_vulnerability_name TEXT, 
    kev_required_action TEXT, 
    kev_known_ransomware_use VARCHAR(32), 
    source_metadata JSON NOT NULL, 
    fetched_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    PRIMARY KEY (cve_id), 
    CONSTRAINT ck_vulnerability_intel_epss_score CHECK (epss_score IS NULL OR (epss_score >= 0 AND epss_score <= 1)), 
    CONSTRAINT ck_vulnerability_intel_epss_percentile CHECK (epss_percentile IS NULL OR (epss_percentile >= 0 AND epss_percentile <= 1))
);

CREATE INDEX ix_vulnerability_intel_kev ON vulnerability_intel (kev);

CREATE INDEX ix_vulnerability_intel_epss ON vulnerability_intel (epss_score);
...
CREATE TABLE vulnerability_findings (
    ...
    cvss_score FLOAT, 
    cvss_vector TEXT, 
    cvss_source VARCHAR(64), 
    title TEXT, 
    description TEXT, 
    primary_url TEXT, 
    "references" JSON NOT NULL, 
    data_source JSON NOT NULL, 
    evidence_stage VARCHAR(64) NOT NULL, 
    exploit_status VARCHAR(32) NOT NULL, 
    exposure_evidence JSON NOT NULL, 
    intel_snapshot JSON NOT NULL, 
    limitations JSON NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    PRIMARY KEY (finding_id), 
    FOREIGN KEY(scan_id) REFERENCES image_scans (scan_id) ON DELETE CASCADE, 
    FOREIGN KEY(cve_id) REFERENCES vulnerability_intel (cve_id) ON DELETE RESTRICT, 
    CONSTRAINT uq_vulnerability_findings_identity UNIQUE (scan_id, cve_id, target, package_type, package_name, installed_version)
);

CREATE INDEX ix_vulnerability_findings_scan ON vulnerability_findings (scan_id);

CREATE INDEX ix_vulnerability_findings_digest ON vulnerability_findings (image_digest);

CREATE INDEX ix_vulnerability_findings_cve ON vulnerability_findings (cve_id);

CREATE INDEX ix_vulnerability_findings_severity ON vulnerability_findings (severity);

UPDATE alembic_version SET version_num='0008_phase8' WHERE alembic_version.version_num = '0007_phase7';
